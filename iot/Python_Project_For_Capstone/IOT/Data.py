...
def dts_tfluna() :
    ...
    while True:
        ...
        df.to_csv("/home/pc/Desktop/sensor_value.csv", index = False)
        print("p0")
        time.sleep(1)
...
def thermalcam_func() :
    ...
    while True:
        ...
        df = pd.DataFrame(frame.astype(int), columns = [s])   
        df.to_csv("/home/pc/Desktop/thermalcam.csv", index = False)
        print("p1")
...
if __name__ == '__main__':
    ...
    #p3 = Process(target = user_main.activate)
    p0.start()
    p1.start()
    #p2.start()
    #p3.start()
